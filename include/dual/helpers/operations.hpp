/// @file operations.hpp
/// @brief Defines dual number operations and generic operation templates
/// (unary, binary, transform, broadcast).
...
struct binary_operation {
  ...
  using enable_t =
      std::enable_if_t<((is_number_v<T1> && is_number_like_v<T2>) ||
  ...
  auto operator()(const number<T, D1...> &n1,
                  const number<T, D2...> &n2) const {
 ...
 private:
  ...
  void update_dvalues(const indices<Dn...>, const N1 &n1, const N2 &n2,
                      N &n) const {
...
struct transform_binary_operation {
  ...
  template <class T1, class T2>
  using enable_t = std::enable_if_t<((is_array_v<T1> && is_array_like_v<T2>) ||
...
struct broadcast_operation {
  ...
  template <class T1, class T2>
  using enable_t = std::enable_if_t<((is_array_v<T1> && is_number_like_v<T2>) ||
 ...
 protected:
  ...
  /// @brief Helper for applying the operation across all elements of an array
  /// and a scalar.
  ...
  /// @brief Helper for applying the operation across all elements of a scalar
  /// and an array.
